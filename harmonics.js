...
function Gwave(sN){
    ...
    let output = {
        Va : pV * (Math.sin(sinp) + Math.sin(sinp * 2+0.2) * 0.05 + Math.sin(sinp * 3+0.6) * 0.35 + Math.sin(sinp * 5+0.8) * 0.1) * mf,
...
try {
    ...
    fs.writeFileSync('./outHarmonics.png', buffer);
  } catch (error) {
    console.error(error);
  }
